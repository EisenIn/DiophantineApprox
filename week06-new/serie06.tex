...
\exercise{
	...
	Next, assume that ii) is true, and let $x \in \Z^n, y \in \Z$ with
		\[ | x^T \alpha - y | \leq \frac{c_2}{ \| x \|_\infty^n}, \]
	and define the lattice 
		\[ \Lambda := \begin{pmatrix} y & x^T \\ (Qc)\cdot\alpha & (Qc) \cdot I_{n-1}  \\ \frac{1}{Q^n c^{n-1}} &  0^T \end{pmatrix} \cdot \Z^{n+1},\]
	...
	\begin{enumerate}
		...
		\item Pick $c = c_2^{-1/n^2}$ and conclude that i) must hold with $c_1 = n \cdot c_2^{1/n^2}.$
	\end{enumerate}
}
{}
